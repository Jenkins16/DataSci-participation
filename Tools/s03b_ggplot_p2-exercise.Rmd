...
tribble(
  ...
  geom_text(aes(label= time), nudge_y= .1, nudge_x = -.02)
```
...
```{r, fig.width = 5, fig.height = 2}
  ...
  geom_line(aes(group = country, color= gdpPercap), alpha=.1) +
  scale_y_log10() +
  scale_color_continuous(trans= "log", breaks = 10^(1:100), labels= scales::comma_format())
```
